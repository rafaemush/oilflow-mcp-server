...
export const TOOLS: ToolDef[] = [
...

  {
    ...
    description:
      "Fetch and re-verify an OilFlow screening receipt. NO API KEY REQUIRED. Given a receipt id (a UUID, from `receipt_id`/`verify_url` on a verdict, or from a receipt the user was handed), returns the receipt, its canonical payload string and an HMAC-SHA256 signature. The signature proves OilFlow issued the receipt and that it has not been altered since; re-verification is performed by OilFlow at this same keyless endpoint, and it does not allow third-party verification without OilFlow (HMAC is symmetric: the verifying key is OilFlow's signing key). When the response carries signature.ed25519, anyone can verify that OilFlow issued the receipt and that it is unaltered using the Ed25519 public key published at /.well-known/oilflow-receipts.json (scripts/verify_receipt_offline.py in the repo does this); the signature proves issuance and integrity, not that the screen ran against the live lists. Say that to the user rather than implying the receipt can be validated without OilFlow, and say which of the two signatures the response actually carried. `receipt_type` says which kind of record it is: a sanctions_screen carries the list set it checked, a predeal_verdict carries the pre-deal tier plus the depth at which it consulted sanctions data. Read `coverage` back to the user: sanctions-only means PEP screening is not shipped, so a politically exposed person who is not also designated will not be flagged. Use this whenever a user has been HANDED a compliance receipt and wants to know whether it is genuine and what it actually covered.",
    inputSchema: {
      type: "object",
      properties: {
